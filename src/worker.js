...
async function ensureSchema(env) {
  ...
  const tables = [
    `CREATE TABLE IF NOT EXISTS products (id TEXT PRIMARY KEY, name TEXT NOT NULL, category TEXT NOT NULL, description TEXT NOT NULL DEFAULT '', price REAL NOT NULL DEFAULT 0, compare_price REAL NOT NULL DEFAULT 0, stock INTEGER NOT NULL DEFAULT 0, image TEXT NOT NULL, badge TEXT NOT NULL DEFAULT '', featured INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`,
    `CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, name TEXT NOT NULL, email TEXT NOT NULL UNIQUE, salt TEXT, password_hash TEXT, provider TEXT NOT NULL DEFAULT 'email', picture TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`,
    `CREATE TABLE IF NOT EXISTS orders (id TEXT PRIMARY KEY, created_at TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'New', customer_json TEXT NOT NULL, items_json TEXT NOT NULL, subtotal REAL NOT NULL, shipping REAL NOT NULL, total REAL NOT NULL)`,
    `CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT NOT NULL)`,
    `CREATE TABLE IF NOT EXISTS admin_credentials (id INTEGER PRIMARY KEY CHECK (id = 1), email TEXT NOT NULL UNIQUE, salt TEXT NOT NULL, password_hash TEXT NOT NULL, version TEXT NOT NULL, updated_at TEXT NOT NULL)`,
  ...
  const insert = 'INSERT OR IGNORE INTO products (id, name, category, description, price, compare_price, stock, image, badge, featured) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
  await env.DB.batch([
    ...tables.map(sql => env.DB.prepare(sql)),
    ...seeds.map(values => env.DB.prepare(insert).bind(...values))
  ]);
  schemaReady = true;
}
...
async function handle(request, env) {
  ...
  if (path === 'health' && method === 'GET') return json({ ok: true, platform: 'Cloudflare Workers + D1' });
